...
class PrivateApiConfig(BaseModel):
    """私有化平台专属鉴权字段"""

    app_token: str = ""
    app_id: str = ""
    biz_no: str = str(uuid.uuid4())
...
class LlmConfig(BaseModel):
    ...
    private_api: Optional[PrivateApiConfig] = None

    @model_validator(mode="after")
    def check_private_fields(self):
        if self.mode == "private" and self.private_api is None:
            raise ValueError("private 模式下必须配置 private_api")
        return self
